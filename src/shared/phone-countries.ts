/**
 * Country dial codes for the phone field (#1539). ISO 3166-1 alpha-2 →
 * calling code, lifted from country-telephone-data. Names come from
 * `Intl.DisplayNames` and the flag from the ISO code, so this is the only
 * table we carry.
 */

const DIAL_CODES: ReadonlyArray<readonly [string, string]> = [
  ['AF', '93'],
  ['AX', '358'],
  ['AL', '355'],
  ['DZ', '213'],
  ['AS', '1684'],
  ['AD', '376'],
  ['AO', '244'],
  ['AI', '1264'],
  ['AG', '1268'],
  ['AR', '54'],
  ['AM', '374'],
  ['AW', '297'],
  ['AU', '61'],
  ['AT', '43'],
  ['AZ', '994'],
  ['BS', '1242'],
  ['BH', '973'],
  ['BD', '880'],
  ['BB', '1246'],
  ['BY', '375'],
  ['BE', '32'],
  ['BZ', '501'],
  ['BJ', '229'],
  ['BM', '1441'],
  ['BT', '975'],
  ['BO', '591'],
  ['BA', '387'],
  ['BW', '267'],
  ['BR', '55'],
  ['IO', '246'],
  ['VG', '1284'],
  ['BN', '673'],
  ['BG', '359'],
  ['BF', '226'],
  ['BI', '257'],
  ['KH', '855'],
  ['CM', '237'],
  ['CA', '1'],
  ['CV', '238'],
  ['BQ', '599'],
  ['KY', '1345'],
  ['CF', '236'],
  ['TD', '235'],
  ['CL', '56'],
  ['CN', '86'],
  ['CO', '57'],
  ['KM', '269'],
  ['CD', '243'],
  ['CG', '242'],
  ['CK', '682'],
  ['CR', '506'],
  ['CI', '225'],
  ['HR', '385'],
  ['CU', '53'],
  ['CW', '599'],
  ['CY', '357'],
  ['CZ', '420'],
  ['DK', '45'],
  ['DJ', '253'],
  ['DM', '1767'],
  ['DO', '1'],
  ['EC', '593'],
  ['EG', '20'],
  ['SV', '503'],
  ['GQ', '240'],
  ['ER', '291'],
  ['EE', '372'],
  ['ET', '251'],
  ['FK', '500'],
  ['FO', '298'],
  ['FJ', '679'],
  ['FI', '358'],
  ['FR', '33'],
  ['GF', '594'],
  ['PF', '689'],
  ['GA', '241'],
  ['GM', '220'],
  ['GE', '995'],
  ['DE', '49'],
  ['GH', '233'],
  ['GI', '350'],
  ['GR', '30'],
  ['GL', '299'],
  ['GD', '1473'],
  ['GP', '590'],
  ['GU', '1671'],
  ['GT', '502'],
  ['GG', '44'],
  ['GN', '224'],
  ['GW', '245'],
  ['GY', '592'],
  ['HT', '509'],
  ['HN', '504'],
  ['HK', '852'],
  ['HU', '36'],
  ['IS', '354'],
  ['IN', '91'],
  ['ID', '62'],
  ['IR', '98'],
  ['IQ', '964'],
  ['IE', '353'],
  ['IM', '44'],
  ['IL', '972'],
  ['IT', '39'],
  ['JM', '1876'],
  ['JP', '81'],
  ['JE', '44'],
  ['JO', '962'],
  ['KZ', '7'],
  ['KE', '254'],
  ['KI', '686'],
  ['XK', '383'],
  ['KW', '965'],
  ['KG', '996'],
  ['LA', '856'],
  ['LV', '371'],
  ['LB', '961'],
  ['LS', '266'],
  ['LR', '231'],
  ['LY', '218'],
  ['LI', '423'],
  ['LT', '370'],
  ['LU', '352'],
  ['MO', '853'],
  ['MK', '389'],
  ['MG', '261'],
  ['MW', '265'],
  ['MY', '60'],
  ['MV', '960'],
  ['ML', '223'],
  ['MT', '356'],
  ['MH', '692'],
  ['MQ', '596'],
  ['MR', '222'],
  ['MU', '230'],
  ['MX', '52'],
  ['FM', '691'],
  ['MD', '373'],
  ['MC', '377'],
  ['MN', '976'],
  ['ME', '382'],
  ['MS', '1664'],
  ['MA', '212'],
  ['MZ', '258'],
  ['MM', '95'],
  ['NA', '264'],
  ['NR', '674'],
  ['NP', '977'],
  ['NL', '31'],
  ['NC', '687'],
  ['NZ', '64'],
  ['NI', '505'],
  ['NE', '227'],
  ['NG', '234'],
  ['NU', '683'],
  ['NF', '672'],
  ['KP', '850'],
  ['MP', '1670'],
  ['NO', '47'],
  ['OM', '968'],
  ['PK', '92'],
  ['PW', '680'],
  ['PS', '970'],
  ['PA', '507'],
  ['PG', '675'],
  ['PY', '595'],
  ['PE', '51'],
  ['PH', '63'],
  ['PL', '48'],
  ['PT', '351'],
  ['PR', '1'],
  ['QA', '974'],
  ['RE', '262'],
  ['RO', '40'],
  ['RU', '7'],
  ['RW', '250'],
  ['BL', '590'],
  ['SH', '290'],
  ['KN', '1869'],
  ['LC', '1758'],
  ['MF', '590'],
  ['PM', '508'],
  ['VC', '1784'],
  ['WS', '685'],
  ['SM', '378'],
  ['ST', '239'],
  ['SA', '966'],
  ['SN', '221'],
  ['RS', '381'],
  ['SC', '248'],
  ['SL', '232'],
  ['SG', '65'],
  ['SX', '1721'],
  ['SK', '421'],
  ['SI', '386'],
  ['SB', '677'],
  ['SO', '252'],
  ['ZA', '27'],
  ['KR', '82'],
  ['SS', '211'],
  ['ES', '34'],
  ['LK', '94'],
  ['SD', '249'],
  ['SR', '597'],
  ['SZ', '268'],
  ['SE', '46'],
  ['CH', '41'],
  ['SY', '963'],
  ['TW', '886'],
  ['TJ', '992'],
  ['TZ', '255'],
  ['TH', '66'],
  ['TL', '670'],
  ['TG', '228'],
  ['TK', '690'],
  ['TO', '676'],
  ['TT', '1868'],
  ['TN', '216'],
  ['TR', '90'],
  ['TM', '993'],
  ['TC', '1649'],
  ['TV', '688'],
  ['VI', '1340'],
  ['UG', '256'],
  ['UA', '380'],
  ['AE', '971'],
  ['GB', '44'],
  ['US', '1'],
  ['UY', '598'],
  ['UZ', '998'],
  ['VU', '678'],
  ['VA', '39'],
  ['VE', '58'],
  ['VN', '84'],
  ['WF', '681'],
  ['EH', '212'],
  ['YE', '967'],
  ['ZM', '260'],
  ['ZW', '263'],
];

export type PhoneCountry = {
  iso: string;
  dialCode: string;
  name: string;
  flag: string;
};

function flagEmoji(iso: string): string {
  return String.fromCodePoint(
    0x1f1e6 + iso.charCodeAt(0) - 65,
    0x1f1e6 + iso.charCodeAt(1) - 65
  );
}

/** Sorted by English name; 'en' on both server and client so SSR matches. */
export function phoneCountries(): PhoneCountry[] {
  const names = new Intl.DisplayNames(['en'], { type: 'region' });
  return DIAL_CODES.map(([iso, dialCode]) => ({
    iso,
    dialCode,
    name: names.of(iso) ?? iso,
    flag: flagEmoji(iso),
  })).sort((a, b) => a.name.localeCompare(b.name));
}

function isKnownCountry(iso: string | null | undefined): iso is string {
  return !!iso && DIAL_CODES.some(([code]) => code === iso);
}

/**
 * Where the visitor is (Cloudflare's geo-IP, passed in), else the browser
 * locale's region (en-AU → AU), else US. The locale is the language
 * setting, not a location — en-GB in Sydney is common.
 */
export function defaultPhoneCountry(geoCountry?: string | null): string {
  if (isKnownCountry(geoCountry)) return geoCountry;
  const region =
    typeof navigator === 'undefined'
      ? undefined
      : new Intl.Locale(navigator.language).maximize().region;
  return isKnownCountry(region) ? region : 'US';
}

/** Shared dial codes resolve to the country most people mean. */
const PREFERRED: Record<string, string> = {
  '1': 'US',
  '7': 'RU',
  '44': 'GB',
  '61': 'AU',
};

/**
 * Split a typed international number ("+61 4…") into its country and the
 * rest, by longest dial-code match. `current` wins a tie (a Canadian who
 * picked CA and types +1 stays on CA). Null when nothing matches yet.
 */
export function splitDialCode(
  typed: string,
  current?: string
): { iso: string; dialCode: string; national: string } | null {
  const digits = typed.replace(/\D/g, '');
  if (!typed.startsWith('+') || !digits) return null;
  const matches = DIAL_CODES.filter(([, dialCode]) =>
    digits.startsWith(dialCode)
  );
  const longest = Math.max(0, ...matches.map(([, d]) => d.length));
  const candidates = matches.filter(([, d]) => d.length === longest);
  const [iso, dialCode] =
    candidates.find(([c]) => c === current) ??
    candidates.find(([c, d]) => PREFERRED[d] === c) ??
    candidates[0] ??
    [];
  if (!iso || !dialCode) return null;
  return { iso, dialCode, national: digits.slice(dialCode.length) };
}
