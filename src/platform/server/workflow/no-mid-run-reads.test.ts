 ...
 *
 * `WorkflowScopedDb` (src/platform/server/db/scoped.ts) removes read methods from the
...
const WORKFLOW_PATH_BY_BASE: Record<string, string> = (() => {
  ...
  for (const path of WORKFLOW_PATHS) {
    ...
    if (map[base]) {
      throw new Error(
        `Duplicate workflow basename ${base}: ${map[base]} and ${path}`
      );
    }
    map[base] = path;
  }
...
function workflowPath(file: string): string {
  const path = WORKFLOW_PATH_BY_BASE[file];
  if (!path) throw new Error(`Unknown workflow source ${file}`);
  return path;
}
...
function scanFileDetailed(file: string): ScannedRead[] {
  const text = stripCommentLines(readFileSync(workflowPath(file), 'utf8'));
...
function workflowSourceFiles(): string[] {
  return Object.keys(WORKFLOW_PATH_BY_BASE).sort();
}
...
const ALLOWED_LIVE_READS: Record<string, SanctionedRead[]> = {
  ...
  'character-voice-workflow.ts': [
    ...
    {
      read: 'characters.getVoiceVersionById',
      bucket: 'CLAIM-BY-ID',
      why: 'The husk this run holds (`targetVersionId`). A persist-step replay after completeIfLive committed must read that completed row instead of releasing the saved slot (#1715).',
    },
    {
