...
- "examples": EVERY worked example stated in the text, copied VERBATIM: [{"params": {<request params exactly as the endpoint receives them — keys must be Input Schema params>}, "usd": <number>, "quote": "<the sentence from the text>"}]. Include per-unit statements as examples with the smallest concrete request they imply (e.g. "$0.08 per image" → params {} usd 0.08). EVERY cell of a price table (size × quality, resolution × tier) is a worked example: params are that row's size as {"width", "height"} (or its tier) plus that column's quality, usd is the cell. A table cell is already the final price: store it in "tables" exactly as printed and look it up. Rounding rules in the text ("rounded up to the closest hundredth of a cent") apply only to a price the card COMPUTES from a formula, never to a looked-up cell. Never invent an example the text does not support, and leave out one whose inputs no request param can carry (a 2048x2048 reference when the request only carries URLs). An empty list is acceptable when the text states no figure that a request can reproduce.
...
Rules:
1. Bind every lever to a parameter name that appears in the Input Schema, spelled exactly. Use the schema's default values as the card's defaults. Never invent a lever (e.g. "reference_tokens", "output_tokens") for a quantity the request does not carry: derive it from the params that do (a count of "reference_image_urls", a "resolution" tier) or leave it out. Token rates the request cannot predict (prompt length) are not levers; price the canonical table instead.
2. Price what the text says. Do not average, guess or copy a sibling model's rate. A size, tier or shape the text does not price must be ABSENT from the tables so the lookup refuses, rather than filled with a neighbour's value. Never give a "lookup" a "default" — a card that does is rejected.
