 ...
 * src/platform/server/workflow/no-mid-run-reads.test.ts, which also asserts that a read's
...
export type WorkflowScopedDb = WorkflowDomains & {
   ...
   * src/platform/server/workflow/no-mid-run-reads.test.ts fails if another workflow
   * references it.
   */
