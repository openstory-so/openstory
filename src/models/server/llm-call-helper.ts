...
import { llmtrCompatibleApi } from '@/models/llmtr';
import {
  createUsageCapture,
  extractRunError,
...
import { withLlmRateLimitRetry } from './quota-retry';
...
import { getShotPromptChannel } from '@/platform/realtime';
  ...
  const { jsonText, costMicros, keySource } = await step.do(
    ...
    async (): Promise<{
      ...
      return withRegionFallback(modelId, async (model) => {
        ...
        // Gemini/OpenRouter 429s a parallel scene fan-out; backoff inside the
          ...
          try {
            for await (const event of eventStream) {
              ...
              if (
                ...
                if (
                  pendingDelta &&
                  Date.now() - lastEmitAt >= flushIntervalMs
                ) {
                  await flushDelta();
                }
                continue;
