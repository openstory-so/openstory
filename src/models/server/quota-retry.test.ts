...
vi.doMock('./byteplus-observability', () => ({
...
const {
  ...
  isLlmRateLimitError,
  jitteredQuotaDelayMs,
  QUOTA_MAX_ATTEMPTS,
  QUOTA_MAX_DELAY_MS,
  quotaDelayMs,
  withBytePlusQuotaRetry,
  withLlmRateLimitRetry,
} = await import('./quota-retry');
...
describe('quotaDelayMs', () => {
  it('doubles each attempt', () => {
    expect(quotaDelayMs(0)).toBe(2_000);
    expect(quotaDelayMs(1)).toBe(4_000);
    expect(quotaDelayMs(2)).toBe(8_000);
  });

  it('caps so one wait cannot stall a workflow step', () => {
    expect(quotaDelayMs(20)).toBe(QUOTA_MAX_DELAY_MS);
  });
});
...
describe('withBytePlusQuotaRetry', () => {
  ...
  it('retries a quota rejection and returns the eventual success', async () => {
    ...
    try {
      ...
      await vi.advanceTimersByTimeAsync(quotaDelayMs(0));
  ...
  it('gives up after the attempt cap and rethrows the last quota error', async () => {
    ...
    try {
      ...
      await assertion;
      expect(fn).toHaveBeenCalledTimes(QUOTA_MAX_ATTEMPTS);
    } finally {
      vi.useRealTimers();
    }
  });
...
describe('quota backoff instrumentation', () => {
  ...
  it('marks the final event exhausted when the budget runs out', async () => {
    ...
    try {
      ...
      expect(calls).toHaveLength(QUOTA_MAX_ATTEMPTS);
      expect(calls.filter((c) => c.exhausted)).toHaveLength(1);
      expect(calls.at(-1)).toEqual({
        operation: 'motion submit',
        attempt: QUOTA_MAX_ATTEMPTS,
        exhausted: true,
      });
    } finally {
      vi.useRealTimers();
...
describe('jitteredQuotaDelayMs', () => {
  it('doubles each attempt before jitter', () => {
    expect(jitteredQuotaDelayMs(0, () => 0.5)).toBe(2_000);
    expect(jitteredQuotaDelayMs(1, () => 0.5)).toBe(4_000);
    expect(jitteredQuotaDelayMs(2, () => 0.5)).toBe(8_000);
  });

  it('caps so one wait cannot stall a workflow step', () => {
    expect(jitteredQuotaDelayMs(20, () => 0.5)).toBe(QUOTA_MAX_DELAY_MS);
  });
});
...
describe('withLlmRateLimitRetry', () => {
  ...
  it('retries a 429 and returns the eventual success', async () => {
    ...
    try {
      ...
      await vi.advanceTimersByTimeAsync(QUOTA_MAX_DELAY_MS);
      await expect(promise).resolves.toBe('ok');
      expect(fn).toHaveBeenCalledTimes(2);
    } finally {
      vi.useRealTimers();
  ...
  it('exhausts the budget and rethrows', async () => {
    ...
    try {
      ...
      for (let i = 0; i < QUOTA_MAX_ATTEMPTS; i++) {
        await vi.advanceTimersByTimeAsync(QUOTA_MAX_DELAY_MS);
      }
