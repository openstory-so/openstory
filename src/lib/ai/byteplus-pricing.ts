 ...
 * on it, and re-date this line when bumping.
 */
 ...
 *
 * Image bills per image. Seedream 5.0 has two tiers — $0.045 at or below
 * 2.36 megapixels and $0.09 above — and we render at 2K, which straddles the
 * boundary (2048x1152 is 2.36MP exactly; a square 2K is 4.19MP). The card
 * carries the HIGHER tier deliberately: an over-estimate makes the credit gate
 * slightly conservative, while an under-estimate lets a team spend past its
 * balance, which is the failure mode #1069 exists to prevent.
 */
export const BYTEPLUS_RATE_CARD: Record<string, EffectiveFalPricing> = {
  // Seedance 2.0 — $4.30 per 1M tokens (standard pack).
  'dreamina-seedance-2-0-260128': {
    unitPrice: micros(4300),
    unit: '1000 tokens',
  },
