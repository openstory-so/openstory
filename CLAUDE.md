...
- **Asset groups: production and local are per-host; every PR preview shares one group (#1519, #1635).** `aigcGroupName` is `openstory-virtual-<app host>` for production/local (`BYTEPLUS_ASSET_GROUP_ID` pins a group by id). Previews all use `openstory-virtual-preview` — Ark quotas are per-account, so a group per PR did not isolate capacity and orphaned slots when the preview Worker/D1 died. `reconcileBytePlusAssets` (`src/models/server/reconcile-byteplus-assets.ts`, cron `53 * * * *` in both blocks) diffs **this deployment's** group against `byteplus_assets`: an Ark asset the ledger does not know and older than the 45-minute lease window is deleted; a ledger row whose asset is gone is forgotten so the slot counts as free. That sweep is a no-op on previews (a shared group plus one PR's D1 would delete another PR's sheets). Production's same cron also `sweepOrphanedPreviewBytePlusGroups`: leftover `openstory-virtual-pr-*` groups are deleted outright, and assets in the shared preview group / localhost / the pre-host `openstory-virtual` group older than 24h are dropped (no live D1 ledger for those). PR-close CI (`scripts/delete-preview-byteplus-group.ts`) only deletes a leftover `openstory-virtual-pr-<n>-*` group — never the shared preview group. Nothing is deleted from Ark after a video — production assets are a reusable working set, only their lease is released. The preview patch splices the container's `v2` between `v1` and `v3` — wrangler applies migrations in array order after the last-applied tag. **Never fall back**: a failed ingest fails the shot, and an Ark portrait rejection is thrown as an Ark error — a quiet hop to fal (or to a public URL) is how a throttled ingest showed up as a fal error the user could not act on.
