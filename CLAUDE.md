...
- **Browser** (`src/shared/sequence-player/export.ts`, `use-sequence-export`) — Theatre Download/Copy live in the Canvas/Script toggle trailing slot (always visible). Playback prefers a matching export MP4; if none exists and the cut is transmux-compatible, it renders on the server then plays (`ensureCut`). Live canvas stitch is the fallback when the container is missing, the cut is mixed-resolution, the user hits Preview now, or the server render failed. Firefox AAC-missing still falls back to the container (#1402) instead of the #1397 error. A matching `sourceShotsHash` is served from cache and never re-rendered.
