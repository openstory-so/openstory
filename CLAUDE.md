...
**Local HTTPS (optional).** Ports 3000–3009 are reserved for `bun dev` worktrees. Each laptop keeps a private hostname map at `~/.openstory/dev-tunnels.json` (not git). `bun tunnel:provision` allocates ten random `*.openstory.so` names, registers them on a Wrangler named tunnel (ingress `127.0.0.1:3000`…`:3009`), and prints Google OAuth redirect URIs to add by hand. `bun dev` sets `VITE_APP_URL` from that file for the worktree's `PORT`. Press `t + Enter` **once** on the machine to connect the named tunnel. E2E uses port **3020** so it never collides. Details: `docs/developer-guide/local-dev-tunnels.md`.
