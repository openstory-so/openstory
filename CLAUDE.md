...
- **Pricing is a static card**, not `model_pricing` — BytePlus publishes no pricing API. `src/billing/byteplus-pricing.ts` holds dated, advertised (NOT bill-verified) unit prices for billing, each paired with the page's full tariff as a **rate card** (`rate-card/cards/byteplus-*.ts`: per-resolution rates, the with-video tier, Seedream's pixel tiers) for estimates, merged into the effective pricing map at read time so a fresh deploy never bills $0. When Ark is configured, `applyBytePlusRouteAliases` points the fal endpoint ids at the whole Ark entry — unit price AND card, since that is what the request will bill — which is why **no estimator or UI call site needs to know the via**. The Ark cards bind the fal-shaped levers the estimator builds (`resolution`, `duration`, `image_size`), not Ark's `size` template, and key their dimension tables by resolution alone — Ark renders every ratio of a resolution class at the same pixel area (frame jobs send `adaptive_<resolution>`), so a 9:16 shot prices at the 16:9 area rather than refusing. Video bills in tokens (÷1000 for the `1000 tokens` unit); images bill per image. Ark units set `recordFalUsage: false`, so Ark cards get no drift report.
...
**Pricing is DB-only (#1069).** `model_pricing` in D1 is the **only** pricing record — there is no baked-in seed. The daily cron (and `bun scripts/refresh-fal-pricing.ts` locally, needs `FAL_KEY`) fills it with unit prices for **every priced endpoint in fal's catalog** (~1,350; raw unit strings, batches of ≤50 — the pricing API's cap), plus fal's typical-units estimates for the endpoints we actually use and observed medians from our own generations. `bun dev` never fires `scheduled()`, so until the script runs locally the table is empty: estimates gate on the $0.10 floor and billing records $0 (reported via `reportMissingBillingCost`). **A brand-new endpoint id has none of those** (a `units` × $1 catalog stub, no fal history, no samples); its rate card (below) is the estimate until the bill and the samples exist. There is no hand-kept unit table any more: H3 Max's "8 units per 5s clip" and the Seedance token formula are rate cards, and an endpoint with no card, no median and no history estimates **null** — never a sibling's number.

**Rate cards (#1605).** The same Pricing text is also stored as a **rate card** — JSONLogic over a closed vocabulary (`src/billing/rate-card/rate-card.schema.ts`, evaluated by `evaluate.ts`, which refuses rather than coerces) — on `model_pricing.rate_card` with its source hash, verified flag and promo `expires_at`. The nightly refresh (`refreshRateCards` in `refresh-fal-pricing.ts`) fetches each used endpoint's llms.txt Pricing + Input Schema sections (`rate-card-source.ts`; token-priced models also get the size table the playground HTML embeds), skips when the hash is unchanged and no promo end has passed, and otherwise makes ONE `anthropic/claude-opus-5` call on OpenRouter's priority ("fast") service tier — a request field via `openRouterCallOptions`, never a `-fast` model slug (`rate-card-extract.ts`, prompt `billing/rate-card-extraction-chat`, no `outputSchema` — the card is recursive, so the model writes JSON text and zod parses it). The model's output is a trust boundary: every bound param and every example key must be a param the Input Schema declares — the model writes its own examples, so an invented lever would verify itself (hand cards may bind card-level levers like `voice_control`; extractions may not); a `lookup` may not carry a `default`; every worked example in the text must reproduce within 1% or the card is rejected with a warn naming the example; a text with no examples stores the card `unverified`; the default request is bounded to [$0.0001, $50]; a re-extraction that changes the card without the text changing logs the diff; an extraction that stops reading a lever the hand card binds is refused and the hand card kept. The hand cards in `rate-card/cards/` seed the table until a newer verified extraction lands; the BytePlus cards ride `BYTEPLUS_RATE_CARD` in code (BytePlus publishes no fal page) and alias onto the fal ids with the unit price. Extraction is capped at `MAX_RATE_CARD_EXTRACTIONS_PER_RUN` per night (Workers cron wall clock); a rejected text is remembered (`rate_card_source_hash` + `rate_card_attempted_at`) and retried only when it changes or a promo end passes since the attempt, so a persistent rejecter cannot hold a slot; a card past its `expires_at` reads as unverified. A refused card is a warn, not a failed cron; the run throws after the writes only when source fetches fail for more than 25% of used endpoints or more than half the model calls made never answered. `bun scripts/extract-rate-card.ts <endpointId>` runs one endpoint by hand; `/admin/rate-cards` shows every card with its evaluator results. Cards are pre-flight estimates only — billing stays `unitsBilled × verified unitPrice`.

**Estimating with a card.** `estimateFalCost` takes the **request** the caller is about to send (`FalCostEstimateParams.request` — the built fal body from `buildMotionRequest` / `buildImageRequest`, or the levers a pre-flight gate knows spelled with the endpoint's own param names) and, when the row carries a verified, unexpired card, evaluates it against that request first; a card the caller hands no request is skipped, not priced at its defaults. Precedence after that is unchanged: observed median (`MIN_OBSERVED_SAMPLES`), fal history, null. A card that refuses the request (a size outside its table) warn-logs and falls through. **Calibration:** every fal usage sample now records its request's price levers (`model_usage_observations.request_params`, reduced by `pricingLevers` — enums, numbers, booleans, list lengths; never a prompt or URL — and never copied into the ledger). The hourly reconcile (`calibrateRateCards`) replays those samples through the card and takes the median of `(unitsBilled × bill-verified unitPrice) / card(levers)`, stores it as `rate_card_calibration` (+ sample count, reset when a new card is stored), and emits `rate_card_drift` per endpoint (median, p90, refused count; warn outside [0.75, 1.33]). The estimator multiplies the card's price by that ratio once `MIN_OBSERVED_SAMPLES` back it, so a "roughly" page becomes calibrated rather than replaced; a ratio outside the band means the card misreads the page, so the card is skipped (never scaled by a misread) and the unit counts take over. Reference clips are a card-level lever — `videoInputLever` adds `input_video_duration` to the levers, never to the body — and a clip sample with no seconds is refused by calibration, as is a card past its promo end. GPT Image 2.5 (t2i and edit) quotes the canonical row of the request's size band, since the app sends presets and tier pixels the table does not list. Rows whose unit price is not bill-verified are not compared (the page against itself). Report + calibration only; the ledger is never adjusted. Rationale and schema: `docs/architecture/rate-cards.md`.
