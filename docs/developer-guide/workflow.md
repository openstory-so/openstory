...
**One take per SCENE, sliced per shot (#1657).** Text to Dialogue acts the turns it is given against each other, so a shot recorded alone is a cold read of a reply the model never heard. The scene's whole conversation is recorded in one call and each shot's clip is cut from it at the provider's own per-turn voice segments (`recordDialogueTake`, `src/motion/server/record-dialogue-take.ts`):

- **The lines come from the scene dialogue node**, not the script: `scene_dialogue_versions` holds them per scene, each naming its shot by id (`SceneDialogueLine.shotId`). The shot-list pass seeds a `prompt` row in `persist-scenes`; a prompt-editor edit appends `user-edit`; a scene with no row yet is derived from its script by `deriveSceneDialogueLines`. The lines are snapshotted onto the run (`GenerationCheckpoint.dialogueLinesBySceneId`) and re-read from D1 at a continue (`refreshCheckpointFromCast`), so an edit made while the run was stopped is what gets recorded.
- **Two indexes.** `SceneVoicedLine.lineIndex` is the scene position a take segment names; `index` stays **shot-relative**, which is what lets every #1554/#1651 helper (`dialogueClipSourceKey`, `matchingDialogueClips`, `spokenLines`, the rewrite merge) work unchanged on a slice.
- **Chunking.** A scene over `DIALOGUE_TAKE_CHUNK_CHARS` (2000 characters of `ttsUtterance` text, ElevenLabs' reliability line for v3) is split at a **shot boundary**, never inside a shot, and the chunk WAVs are joined by `concatWavs` with the segment times offset.
- **Bytes never cross a step (#1645).** Each chunk is recorded and parked in R2 inside its own `step.do`; the assemble step reads the chunks back by key, joins, slices (`sliceWav`), and uploads the take plus each shot's clip. Only `{ url, durationSeconds, segments, … }` records ride the checkpoint.
- **Slice boundaries.** The silence between two turns belongs to the shot that is about to speak, so a clip starts where the previous shot stopped speaking and runs to the next shot's first word (`shotSliceWindows`); its tail then comes off with the same rule as before and it is padded up to the provider floor.
- **Persistence.** `scene_dialogue_takes` records the take (url, duration, segments, the per-shot `clips`, `inputHash` = `dialogueTakeKey`), one selected row per scene; `shots.audioClips` keeps the slices as the working set, each stamping `takeId`. A re-run skips a scene whose shots all carry matching clips from one take. Selecting another take (`selectSceneDialogueTakeFn`) puts that take's slices back on the shots.

**Fitting the take to the clip (#1651).** ElevenLabs v3 takes no target or maximum duration, so the length is discovered rather than requested. The ladder runs **per shot slice** over a scene-wide recording: a slice over its shot's limit sends THAT shot's turns to the rewrite and the whole scene is re-recorded. `fitDialogueClip` (`src/motion/server/fit-dialogue-clip.ts`) is the per-shot twin, still used by `MotionWorkflow`'s standalone-synthesis path; both share `shortenDialogueLines`, so the rungs behave identically:
