...
- **Chunking.** A conversation over `DIALOGUE_TAKE_CHUNK_CHARS` (2000 characters of `ttsUtterance` text, ElevenLabs' reliability line for v3) is split at a **shot boundary**, never inside a shot (`chunkTakeLines`). Each chunk is its own recording — nothing is joined — and only chunks that contain an adopting shot are recorded. A chunk also breaks where the voice provider changes (a Seed voice and an ElevenLabs voice never share a call), and a Seed chunk where a third speaker would join or past 1,000 line characters — a 3+ speaker Seed scene is recorded as a series of two-person exchanges.
