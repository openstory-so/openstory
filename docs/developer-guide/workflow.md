...
1. **Snap durations** — Snaps scene durations to video model capabilities upfront for the motion prompts. Music does not use them: its scene summaries come from `musicSceneSummariesFromAnalysis`, one row per scene with that scene's shot durations summed and its head shot's visual prompt text (`visualSummaryBySceneId` on the payload), built through the same insert builders that wrote the `scenes` / `shots` rows. Verify, regenerate and Update all rebuild them from those rows and each head shot's selected visual prompt with `loadMusicSceneSummaries`, so a pipeline music prompt reads fresh the moment it lands (#1783)
