...
- **A line at the mic (#1802).** `DialogueTakeWorkflow` (`src/motion/server/workflows/dialogue-take-workflow.ts`, trigger `/dialogue-take`, from `recordShotDialogueLineFn`) is the same claim → record → cut → `appendRecording` lifecycle for one shot. Its one recording step (`recordDialogueTake`) turns the user's take into the speaker's voice — ElevenLabs Voice Changer, or Seed Audio with the take as a delivery reference — and splices it into the shot's current reading (snapshotted on the payload as `base`), then lands it as a `mic` reading. It refuses a result over `dialogueFitBudget`'s limit rather than rewriting the user's words.
