...
- **Two indexes.** `SceneVoicedLine.lineIndex` is the running position in the conversation that was sent, which a recording turn names; `index` stays **shot-relative**, which is what lets every #1554/#1651 helper (`dialogueClipSourceKey`, `matchingDialogueClips`, `spokenLines`, the rewrite merge) work unchanged on one shot's lines.
...
1. `convertWithTimestamps` (not `convert`) returns the recording plus a character alignment and per-turn voice segments. `trimmedEndSeconds` pulls the section's end back to whichever is LATER of the last audible sample and the alignment end — so an alignment that under-reports cannot clip a word, and a noise floor that never dips cannot keep a tail the alignment says is silent. It measures in place and moves a number, not bytes; the pad up to the provider floor is added later, at cut time.
