...
- Usage queries (`list_shot_references`, `list_entity_usages`) answer from the editor inspector's own facet resolution (`loadSceneFacets`): one batch of sequence-wide reads, matched against the whole bible, then paged in memory. A page holds matches only.
