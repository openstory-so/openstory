...
An element's token is a label (#1827). The still, visual-prompt and motion-prompt hashes read text through one normaliser (`elementTokensToKeys` in `src/shots/input-hash.ts`) that swaps each element token for the element's identity: its row id on a still (the snapshot's `elementTokens`), its description in a prompt, whose element bible comes from the analysis LLM and has no id. The prompt element bible is hashed without its token. So a rename (`cascadeRename`) stales nothing (the clip follows `renamedFromId`, below), while a new description, a new element image or a script that names a different element still does. Only the hash is normalised; models still receive the tokens. Verify accepts the raw-token digests stamped before #1827 until `LEGACY_HASH_UNTIL`, so untouched work stays fresh; work renamed before the deploy reads stale once.
