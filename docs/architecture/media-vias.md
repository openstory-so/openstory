...
Seedance has neither: Ark documents no limit, only a style recommendation ("no more than 500 Chinese characters or 1,000 English words"), and no fal Seedance schema declares `maxLength`. The 4096 we carried was our own number and was quietly cutting prompts; the recommendation is now Ark's own 1,000 words, and it is **counted in words** (`promptLengthUnit: 'words'` on the catalog entry; `measurePrompt` / `promptLengthUnit` in `src/models/prompt-length.ts` are the only readers, so the counter, the warning and the log all agree). Every other model counts characters. H3 Max's 2500 was invented the same way and is now fal's 50000.
