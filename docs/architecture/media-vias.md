...
**Audio refs upload to fal storage.** Image and video refs stay on the R2 CDN (`R2_PUBLIC_STORAGE_DOMAIN`). Audio refs (dialogue clips, uploaded audio elements, studio `referenceAudio`) are read from R2 at submit time and `fal.storage.upload`'d, because MiniMax H3 fails to fetch our custom-domain URLs. Pass `{ uploadToFalStorage: true }` to `ensureExternallyFetchableUrl`. E2E replay still passes the relative `/r2/` URL through. Canonical bytes stay in R2.
