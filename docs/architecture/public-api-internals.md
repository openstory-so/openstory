...
- **Each clip gets a fragmented copy, once, at ingest.** HLS cannot point at a plain MP4 (`moov` + one `mdat`), which is how clips are stored. `uploadVideoFromUrl` and a user shot-video finalize write `<clip>.frag.mp4` beside the original — the same packets repackaged, nothing decoded — then `<clip>.frag.json` (init length, size, duration, codec, has-audio). The sidecar is written last; its presence says the copy is whole. The copy is written by `fragment-mp4.ts`, not a muxer: the init section (`ftyp` + `moov`/`mvex`) and the one `moof` are built from the source's `moov` (a few KB; codec boxes copied verbatim, edit list applied, pre-zero AAC priming dropped), and the sample bytes are copied from the source as coalesced ranged reads (≤ 4 MiB each) straight into fixed 8 MiB multipart parts. Memory is one read + one part whatever the clip length — **a whole clip must never sit in Worker memory**. Do not go back to mediabunny's fragmented muxer for this: it holds every sample until a key frame closes the fragment, and a generated clip usually has one key frame, so it buffered the whole clip and wrote nothing until `finalize`; #1737 held an `r2.put` open across that and R2 dropped it ("Network connection lost", a 30 s Seedance clip on the Videos page, 2026-09-26). Images / Videos page clips never play in the theatre and get no copy (`theatreCopy: false`). `GET theatre.m3u8` only reads sidecars. A missing sidecar (an older clip) 400s and the theatre stitches.
