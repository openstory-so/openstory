...
- **Each clip gets a fragmented copy, once.** HLS cannot point at a plain MP4 (`moov` + one `mdat`), which is how clips are stored. The first playlist that names a clip writes `<clip>.frag.mp4` beside it — the same packets repackaged in the worker, nothing decoded — then `<clip>.frag.json` (init length, size, duration, codec, has-audio). The sidecar is written last; its presence says the copy is whole. Two at a time: each holds a clip in and out of memory.
