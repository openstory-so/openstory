...
| Character/location sheet | **Two hashes by design.** The PARENT (`characters.sheetInputHash` / `sequence_locations.referenceInputHash`) gets the verify-mirrored current-inputs hash (bible + talent/library ref + style + model) so later edits re-stale the sheet. Each VERSION row is append-only; `selectedSheetVersionId` / `selectedReferenceVersionId` is the live pointer (mirrored onto the parent URL). Uploads stamp the version as `upload:<storagePath>`. Stills hash the selected version id when present, else the parent input hash — so a new sheet image re-stales stills even when bible inputs didn't change. | Legacy rows with no selection pointer keep hashing the parent input hash (no catalog-wide untracked wipe).                                        |
