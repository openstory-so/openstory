...
| ------------------------------------------------------- | ----------------------------------------------------------------------------------------- | -------------------------------------------------- |
| Scene script / title / location / timeOfDay / storyBeat | Visual + motion prompts of the scene's shots                                              | Sheets (no script→bible edge — locked decision #3) |
...
| Character/location bible field edit                     | Prompts of referencing scenes (projected fields); the entity's sheet                      | Unreferenced entities' artifacts (narrowing)       |
...
| Sheet manual upload                                     | The parent's `sheetInputHash` restamps from current inputs (see §1.4 caveat)              | Prompts; stills when inputs are unchanged          |
...
| **B** still only       | `setFrameImageFromUploadFn` | Append `frame_variants` `kind:'upload'` stamped against the **current** selected prompt + sheets, then `frameVariants.select` (the exact `setImageFromVariantFn` repoint). Video stale by derivation.                                                                                                                                 |
...
| Character/location sheet | **Two hashes by design.** The promoted PARENT (`characters.sheetInputHash` / `sequence_locations.referenceInputHash`) gets the verify-mirrored current-inputs hash (bible + talent/library ref + style + model) so later edits re-stale it. The VARIANT row's own `inputHash` is content-addressed to the upload (`upload:<storagePath>`, unique per upload) so re-uploads never collide on the divergent unique key and each upload keeps an undo-able history row. | **Caveat (deliberate):** with unchanged inputs the parent hash VALUE equals the generated one, so existing stills do not auto-stale from the upload alone — replacing stills is the user's next manual step (consistent with "identical inputs don't churn dependents"). |
...
| `setFrameImageFromUploadFn`                                                                       | `{…, publicUrl}` → `{shotId, versionId, thumbnailUrl}`                                          | §1.3 B                                                           |
| `replaceFrameContentFn`                                                                           | `{…, frameId?, promptText?, publicUrl}` → `{shotId, versionId, promptVersionId, promptChanged}` | §1.3 C (atomic)                                                  |
...
| `setCharacterSheetFromUploadFn` / `setLocationSheetFromUploadFn`                                  | `{sequenceId, characterId/locationDbId, publicUrl}` → updated row                               | §1.4 sheets (variant + promote flow)                             |
...
- **Prompt-hash version bumps have a deploy step.** Changing the hashed body
  shape requires bumping `PROMPT_INPUT_HASH_VERSION`
  ...
  `drizzle/migrations/20260828031159_null_prompt_hashes_for_v5`.
