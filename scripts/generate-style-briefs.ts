...
type Flags = {
  ...
  out: string | null;
  concurrency: number;
  /** Briefs per style. >1 writes the variants module instead. */
  variants: number;
};
...
function parseFlags(argv: string[]): Flags {
  ...
  return {
    ...
    variants: Math.max(1, Math.round(num('--variants', 1))),
  };
}
...
- When asked for several briefs, each must be a genuinely DIFFERENT idea for the same style — different subject, setting and event. Rewordings of one idea are useless.

Return ONLY JSON: { "briefs": ["<sentence>", …] }`;
...
const briefsSchema = z.object({
  briefs: z.array(z.string().min(10)).min(1),
});
...
async function generateBriefs(
  ...
  const res = await fetch(OPENROUTER_URL, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        {
          role: 'user',
          content: `${styleBlock(style)}\n\nWrite ${count} brief${count === 1 ? '' : 's'}.`,
        },
      ],
    }),
  });
  ...
  const { briefs } = briefsSchema.parse(JSON.parse(text.slice(start, end + 1)));
  return briefs.slice(0, count).map((brief) => brief.trim());
}
...
async function main() {
  ...
  const briefs: {
    ...
    try {
      const written = await generateBriefs(style, flags.model, flags.variants);
      console.log(
        `• ${style.name} (${slug})\n${written.map((b) => `    ${b}`).join('\n')}\n`
      );
      return { slug, name: style.name, briefs: written };
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.error(`⚠️  ${style.name} (${slug}): ${message}\n`);
      return { slug, name: style.name, briefs: [], error: message };
    }
  });

  const ok = briefs.filter((b) => b.briefs.length > 0);
  ...
  if (flags.out) {
    const multi = flags.variants > 1;
    const record = Object.fromEntries(
      ok.map((b) => [b.slug, multi ? b.briefs : b.briefs[0]])
    );
    const body = multi
      ? '// AUTO-GENERATED by scripts/generate-style-briefs.ts --variants — do not edit by hand.\n' +
        ...
        ' * @public\n' +
        ' */\n' +
        `export const STYLE_BRIEF_VARIANTS: Record<string, string[]> = ${JSON.stringify(record, null, 2)};\n`
    ...
    console.log(
      `📄 ${path.relative(process.cwd(), outPath)} (${ok.length} styles × ${flags.variants})`
    );
  }
}
